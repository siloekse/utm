...
def meridian_convergence(latitude, longitude, zone_number=None):
    """This function computes the deviation in between true north and UTM grid north at a given latitude and longitude.
    This is called meridian convergence.
     ...
     Returns
     -------
     meridian_convergence: float or NumPy array
         Deviation between true north and UTM grid north.

    """
    if zone_number is None:
        zone_number = latlon_to_zone_number(latitude, longitude)

    central_meridian = zone_number_to_central_longitude(zone_number)

    sin_lat = mathlib.sin(mathlib.radians(latitude))
    tan_lon = mathlib.tan(mathlib.radians(longitude - central_meridian))

    return mathlib.degrees(sin_lat * tan_lon)
